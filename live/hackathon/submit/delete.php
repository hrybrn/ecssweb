...
if($type == 'person'){
    //get person id
    $sql = "SELECT hp.hackathonPersonID AS id
    ...
    if(!$person = $statement->fetchObject()){
        $failed = true;
    } else {
        $params = [':personID' => $person['id']];

        $sql = "DELETE FROM hackathonTeam WHERE hackathonTeamLeaderID = :personID;";

        $statement = $db->prepare($sql);
        $statement->execute($params);

        $sql = "DELETE FROM hackathonPerson WHERE hackathonPersonID = :personID;";
    }
}
...
echo json_encode(['status' => true, 'message' => 'deleted correctly']);